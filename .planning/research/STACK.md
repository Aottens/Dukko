...
**Domain:** VST3 audio plugin (tempo-synced volume-ducking effect, Kickstart-class) — Apple Silicon first, possible Windows + commercial release later
**Researched:** 2026-05-03
**Confidence:** HIGH (frameworks and tooling in this domain are well-known and stable; license terms verified)
...
**One-line stack summary:** *JUCE 8 + CMake + CPM + C++20, with `clap-juce-extensions` for CLAP and `chowdsp_utils` for DSP polish, validated with pluginval and Steinberg's tool, built on a `macos-14` GitHub Actions runner.*
...
Use **JUCE 8** (current 8.0.x series). It is the prescriptive correct choice for this project.

### Rationale

**License compatibility with possible commercial release** — VERIFIED HIGH:
...
**GUI toolkit** — HIGH for this project's needs:
...
**Maturity, examples, learning resources** — HIGH:
...
- C++20 brings genuine quality-of-life features for this project:
  - `std::span` for non-owning audio buffer views,
  - Concepts (`std::floating_point`) for cleaner DSP templates,
  - Designated initializers (`{.depth = 0.5f, .mix = 1.0f}`) for parameter struct construction,
  - `<numbers>` (`std::numbers::pi_v<float>`) for DSP constants.
...
Rust would force a complete framework rewrite (no JUCE), no Bitwig-tested template at this complexity level, and the developer's stated workflow (Claude builds) has dramatically more JUCE+C++ training data to draw on than nih-plug+Rust. Saved for a future project, not this one.
...
Use JUCE built-ins as the foundation; reach for `chowdsp_utils` for parameter polish and shared utilities.
...
- Modules of interest for this project:
  - `chowdsp_parameters` — improved `RangedAudioParameter` subclasses with smoothing built in;
  - `chowdsp_presets_v2` — preset save/load infrastructure (handles the JSON-on-disk side of FEATURES.md's curve import/export);
  - `chowdsp_plugin_state` — versioned state serialization (FEATURES.md correctly flags this needs versioning).
...
**Roll your own** as a `juce::Component` modeled on the Xfer LFOTool / Cableguys VolumeShaper "point + tension" model. There is no acceptable off-the-shelf component, and the data model is small enough that a custom one is the right call.
...
```cpp
struct CurvePoint {
    float position;   // 0..1, normalized along curve length
    float value;      // 0..1, where 0 = full duck, 1 = no duck
    float tension;    // -1..+1, shape of the segment AFTER this point
};
struct Curve {
    std::vector<CurvePoint> points; // sorted by position; first.position==0, last.position==1
};
```
...
jobs:
  build-macos-arm64:
    ...
    steps:
      ...
      - name: Validate VST3 (strictness 10)
        run: ./pluginval.app/Contents/MacOS/pluginval \
                 --strictness-level 10 \
                 --validate build/KickstartClone_artefacts/Release/VST3/KickstartClone.vst3

      - name: Upload VST3 artifact
        uses: actions/upload-artifact@v4
        with:
          name: KickstartClone-vst3-macos-arm64
          path: build/KickstartClone_artefacts/Release/VST3/KickstartClone.vst3
```
...
**Yes — ship CLAP alongside VST3 in v1.**

### Cost (with JUCE 8)

JUCE 8 does NOT have native CLAP support. JUCE 9 will. For now, use **`clap-juce-extensions`** (https://github.com/free-audio/clap-juce-extensions, MIT-licensed) which adds a CLAP build target to a JUCE plugin without changing your DSP/UI code.
...
For ~half a day of CMake work and zero ongoing maintenance cost, you get a Bitwig-native plugin format. **Ship CLAP in v1.**
...
git clone --recurse-submodules https://github.com/sudara/pamplejuce KickstartClone
cd KickstartClone
...
cp -R build/KickstartClone_artefacts/Release/VST3/KickstartClone.vst3 \
      ~/Library/Audio/Plug-Ins/VST3/
cp -R build/KickstartClone_artefacts/Release/CLAP/KickstartClone.clap \
      ~/Library/Audio/Plug-Ins/CLAP/

# 5. Validate locally
./pluginval.app/Contents/MacOS/pluginval --strictness-level 10 \
      --validate ~/Library/Audio/Plug-Ins/VST3/KickstartClone.vst3
```
...
CPMAddPackage(
    ...
    GIT_TAG 8.0.4   # pin to a specific 8.x release
)
...
clap_juce_extensions_plugin(
    TARGET KickstartClone
    CLAP_ID "com.aottens.kickstartclone"
...
**If commercial release becomes committed (decision in v2):**
...
**If multiband ducking is added (v2):**
...
*Stack research for: VST3 audio plugin (tempo-synced volume-ducking effect, Kickstart-class)*
*Researched: 2026-05-03*
